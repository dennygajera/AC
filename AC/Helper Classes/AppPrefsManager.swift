...
extension DefaultsKeys {
    var userData: DefaultsKey<LoginUserData?> { .init("user") }
    var defaultappData: DefaultsKey<DefaultValues?> { .init("defaults") }
    var userBalance: DefaultsKey<UserBalance?> { .init("userbalance") }
    var userId: DefaultsKey<Int?> { .init("userId") }
    var operatorId: DefaultsKey<String?> { .init("operatorId") }
...
class AppPrefsManager: NSObject {
    ...
    let MYPROFILETAB = "MyProfileTab"
    let OTHERPROFILETAB = "OtherProfileTab"
    let CARDSTATUS = "CardStatus"
    let BALANCE = "Balance"
    let PROFILETHUMB = "ProfileThumb"
    ...
    func getUserId() -> Int? {
        let strToken = getDataFromPreference(key: USERID)
    ...
    func setMyProfileTab(obj: Int) {
        setDataToPreference(data: obj as AnyObject, forKey: MYPROFILETAB)
    }
    
    func getMyProfileTab() -> Int? {
        let tab = getDataFromPreference(key: MYPROFILETAB)
        return tab as? Int
    }
    
    func setCardStatus(obj: String) {
        setDataToPreference(data: obj as AnyObject, forKey: CARDSTATUS)
    }
    
    func getCardStatus() -> String? {
        let tab = getDataFromPreference(key: CARDSTATUS)
        return tab as? String
    }
    
    func setProfileThumb(obj: String) {
        setDataToPreference(data: obj as AnyObject, forKey: PROFILETHUMB)
    }
    
    func getProfileThumb() -> String? {
        let link = getDataFromPreference(key: PROFILETHUMB)
        return link as? String
    }
    
    func setOtherProfileTab(obj: Int) {
        setDataToPreference(data: obj as AnyObject, forKey: OTHERPROFILETAB)
    }
    
    func getOtherProfileTab() -> Int? {
        let tab = getDataFromPreference(key: OTHERPROFILETAB)
        return tab as? Int
    }
}
